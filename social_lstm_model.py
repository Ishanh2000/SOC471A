    ...
    def _load_glove_embeddings(self):
        ...
        with open(constants.WORD_EMBEDS) as fp:
            ...
            for i, line in enumerate(fp):
                embeddings[i,:] = [float(x) for x in line.split()[1:]] # map(float, line.split()[1:])
        return embeddings
...
def train(model, train_data, val_data, test_data, optimizer, 
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_data):
            ...
            if ema_loss is None:
                ema_loss = loss.data # loss.data[0]
            else:
                ema_loss = 0.01*loss.data + 0.99*ema_loss # 0.01*loss.data[0] + 0.99*ema_loss

            if i % 10 == 0:
                print (epoch, i, ema_loss)
                # print >>log_file, epoch, i, ema_loss
                log_file.write(f"{epoch}, {i}, {ema_loss}\n")
            if  i % log_every == 0:
                auc = evaluate_auc(model, val_data)
                print ("Val AUC", epoch, i, auc)
                if not log_file is None:
                    # print >>log_file, "Val AUC", epoch, i, auc
                    log_file.write(f"Val AUC: {epoch}, {i}, {auc}\n")
                if auc > best_iter[0]:
    ...
    if not log_file is None:
        ...
        log_file.write(f"Overall best test: {best_test}\n")
        log_file.write(f"Overall best val : {best_iter}\n")
